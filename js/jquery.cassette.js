...
( function( window, $, undefined ) {
	...
	$.Cassette.defaults 	= {
		...
		songs			: [{
            sources:{
                mp3:"songs/BlueDucks_FourFlossFiveSix.mp3",
                ogg:"songs/BlueDucks_FourFlossFiveSix.ogg"
            }
        },{
            sources:{
                mp3:"songs/BlankKytt_ThursdaySnowReprise.mp3",
                ogg:"songs/BlankKytt_ThursdaySnowReprise.ogg"
            }
        },{
            sources:{
                mp3:"songs/BlueDucks_FlossSuffersFromGammaRadiation.mp3",
                ogg:"songs/BlueDucks_FlossSuffersFromGammaRadiation.ogg"
            }
        },{
            sources:{
                mp3:"songs/BlankKyt_RSPN.mp3",
                ogg:"songs/BlankKyt_RSPN.ogg"
            }
        }],
	...
	$.Cassette.prototype 	= {
		...
		_init				: function( options ) {
			...
			this.cntTime		= 0;
			// current sum of the duration（持续的） of played songs 
			this.timeIterator	= 0;
			// used for rewind / forward，elapsed（消逝）
			this.elapsed		= 0.0;
			// action performed（表现）
			this.lastaction		= '';
		...
		// songs are distributed（散发; 分配） equally on both sides
	...
	$.Song.prototype 		= {
		...
		_init				: function() {
            this.sources	= this.song.sources;
		},
		...
		// load metadata to get the duration（持续的时间） of the song
	...
	$.fn.cassette			= function( options ) {
		...
		if ( typeof options === 'string' ) {
            ...
            //等价源码，var args = arguments.slice(1);
            //分离参数（方法，参数，参数）
			var args = Array.prototype.slice.call( arguments, 1 );
			//console.log(args);
            //this指的是，$('#vc-container')，被选择器选中的所有元素集合
			this.each(function() {
                //this指的是元素集合中的单个元素，详情见jquery each方法
                //向本元素取data-cassette对应的值
				var instance = $.data( this, 'cassette' );
                
                //检测插件有没有初始化，没有初始化的情况下调用方法会报错
				if ( !instance ) {
                    //prior（优先的）
				...
				//如果初始化成功，会取出对象中的"options"方法，并判断
                //options.charAt(0) === "_"，不允许调用内部私有方法
